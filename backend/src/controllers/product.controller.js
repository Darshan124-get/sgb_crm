...
exports.createProduct = async (req, res) => {
    console.log('[DEBUG] Product Create Body:', JSON.stringify(req.body));
    console.log('[DEBUG] Product Create File:', req.file ? req.file.filename : 'No file');

    const { 
        category_id, description, unit, selling_price, 
        dealer_price, min_stock_alert, status, 
        discount_percentage, opening_stock 
    } = req.body;

    // Be more thorough with name extraction
    ...
    // Store image in DB as Base64 if a file is uploaded
...
exports.updateProduct = async (req, res) => {
    const { 
        ...
        dealer_price, min_stock_alert, status, discount_percentage 
    } = req.body;
