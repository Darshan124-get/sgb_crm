...
async function loadMediaFiles() {
    try {
        ...
        const res = await fetch(`${window.API_URL}/chatbot/media`, {
...
function renderMediaTable() {
    ...
    tableBody.innerHTML = filtered.map(file => {
        ...
        const mediaUrl = `${window.BASE_URL || 'http://localhost:5000'}/media/uploads/${file.filename}`;
        ...
        return `
            <tr id="media-row-${file.id}">
                ...
                <td>${file.date}</td>
                <td>
                    <button class="btn-action" onclick="copyMediaLink('${mediaUrl}')" title="Copy Media URL"><i class="fa-solid fa-link"></i></button>
...
function simulateUpload() {
    ...
    if (selectedFile) {
        ...
        uploadFilename = name.toLowerCase().replace(/\s+/g, '_') + '.pdf';
    }

    closeUploadModal();
    window.showAlert("Uploading File", "Uploading media to Server...", "info");
